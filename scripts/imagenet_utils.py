...
def map_darknet_labels(labels, order):
    ...
    ilsvrc_synset_path = os.path.join(
        metadata_root, 'imagenet_lsvrc_2015_synsets.txt')
    darknet_synset_path = os.path.join(
        metadata_root, 'darknet_lsvrc_synsets.txt')
    ...
    if order == 'darknet2ilsvrc':
        map_from_darknet_to_ilsvrc = [
            ilsvrc_synsets.index(x) for x in darknet_synsets]
        map_f = np.vectorize(lambda x: map_from_darknet_to_ilsvrc[x])
    elif order == 'ilsvrc2darknet':
        map_from_ilsvrc_to_darknet = [
            darknet_synsets.index(x) for x in ilsvrc_synsets]
        map_f = np.vectorize(lambda x: map_from_ilsvrc_to_darknet[x])
    else:
        raise ValueError(
            "Pick map_darknet_labels order from: ['darknet2ilsvrc', 'ilsvrc2darknet']")
...
def inception_preprocess_input_fn(image, output_height, output_width, is_training=False, fast_mode=True):
    if is_training:
        ...
        if image.dtype != tf.float32:
            # Returns fp32 (with range [0, 1))
            image = tf.image.convert_image_dtype(image, dtype=tf.float32)
        distorted_image, distorted_bbox = _distorted_bounding_box_crop(
            image, bbox)
        ...
        if image.dtype != tf.float32:
            # Returns fp32 (with range [0, 1))
            image = tf.image.convert_image_dtype(image, dtype=tf.float32)
        # Crop central region containing 87.5% of original image
...
def vgg_preprocess_input_fn(image, output_height, output_width, is_training=False):
    if is_training:
        resize_side = tf.random_uniform(
            [], minval=_RESIZE_SIDE_MIN, maxval=_RESIZE_SIDE_MAX+1, dtype=tf.int32)
...
def inception_caffe2tf_preprocess_input_fn(image, output_height, output_width, is_training=False):
    if is_training:
        ...
        image = tf.image.resize_images(
            image, [output_height, output_width])  # Default = bilinear
        # RGB -> BGR ;  image dims: (H, W, C)
        image = tf.reverse(image, axis=[-1])
        image = _mean_image_subtraction(image, [_B_MEAN, _G_MEAN, _R_MEAN])
    return image
...
def dataset_input_fn(filenames, model_dir, image_size, batch_size, num_threads, shuffle=False, num_epochs=None, initializable=False, is_training=False):
    ...
    def _parse_function(example_proto):
        ...
        # Returns uint8 (with range [0, 255])
        image = tf.io.decode_jpeg(parsed_features['image/encoded'], channels=3)
        if re.match('.*inception.*slim.*', model_dir):
            # Center crop, resize, scale and shift [0, 255] -> [0, 1] -> [-1, 1]
            image = inception_preprocess_input_fn(
                image, image_size, image_size, is_training)
            ...
            image = darknet_preprocess_input_fn(
                image, image_size, image_size, is_training)
...
def images_input_fn(filenames, model_dir, image_size, batch_size, num_threads, shuffle=False, num_epochs=None, initializable=False, is_training=False):
    def _parse_function(filename):
        ...
        image = tf.io.decode_image(
            image_string, channels=3, dtype=tf.dtypes.uint8, expand_animations=False)
