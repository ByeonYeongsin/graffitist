...
parser = argparse.ArgumentParser(
    description='TensorFlow ImageNet Validation Script')
...
def validate(val_filenames):
    ...
    num_val_batches_per_epoch = int(
        np.ceil(im_utils.num_examples_per_epoch('validation') / args.batch_size))
    ...
    if args.gen_calib_set:
        ...
        with tf.compat.v1.Session(graph=tf.Graph()) as sess:
            features, _, _ = im_utils.dataset_input_fn(
                val_filenames, args.model_dir, args.image_size, args.calib_set_size, num_threads, shuffle=True, is_training=False)
